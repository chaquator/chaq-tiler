...
static Iterator draw_grid(const Iterator start, const Iterator end, const Rect& area, Vec::vec_t margin,
    ...
    // axis: bit 0, primary direction: bit 1, secondary direction: bit 2
    auto axis = orientation.test(Config::OrientationBits::GridAxis);
    auto primary_reverse = orientation.test(Config::OrientationBits::GridPrimaryDirection);
    auto secondary_reverse = orientation.test(Config::OrientationBits::GridSecondaryDirection);
